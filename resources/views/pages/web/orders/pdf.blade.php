        ...
        <table class="table">
            <tbody>
                <tr>
                    ...
                    <td>{{ Str::ucfirst($item->order->payment->method) }}</td>
                </tr>
                <tr>
                    <td style="padding-right: 35px">Status</td>
                    <td>
                        @if ($item->order->payment->status == 'pending')
                            Menunggu
                        @elseif ($item->order->payment->status == 'approved')
                            @if ($order->payment->method == 'Cash')
                                Dikonfirmasi (Belum Dibayar)
                            @else
                                Dikonfirmasi
                            @endif
                        @elseif ($item->order->payment->status == 'rejected')
                            Ditolak
                        @endif
                    </td>
                </tr>
